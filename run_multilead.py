#!/usr/bin/env python
"""Multi-lead evaluation: persistence / clim / LSTM / ST / hybrid (+ sparse)."""
...
def prepare_lead(splits, lead_index: int, stats):
    def pack(split):
        y = split["y"][:, lead_index]
        x_n = history_to_norm_anom(
            split["x"], split["hist_times"], stats, split["x"].shape[1]
        )
...
def main():
    ...
    parser.add_argument(
        "--sparse",
        choices=["none", "point", "block", "station"],
        default="none",
    )
    ...
    fa = build_forecast_arrays(ds, leads=LEADS_MONTHS)
    ...
    print(
        ...
        f"leads={leads} sparse={args.sparse} device={device}"
    )
    ...
    for li, lead in enumerate(leads):
        data = prepare_lead(splits, li, stats)
        ...
        if args.sparse != "none":
            m_tr = make_batch_masks(
                x_tr, args.sparse, args.keep_ratio, args.stations, seed=SEED + lead
            )
            m_va = make_batch_masks(
                x_va, args.sparse, args.keep_ratio, args.stations, seed=SEED + 100 + lead
            )
            m_te = make_batch_masks(
                x_te, args.sparse, args.keep_ratio, args.stations, seed=SEED + 200 + lead
            )
        ...
        persist_va = persistence_predict(x_va_raw)
        ...
        st = train_st_anom(x_tr, y_tr_n, x_va, y_va_n, mask, device, args.epochs)
    ...
    lines = [
        ...
        "",
        f"Region: `{region.get('id')}` · cube: `{ds.attrs.get('source')}` · sparse=`{args.sparse}`",
        "",
    ...
    if depth_profiles_lead1:
        lines += ["", "## Depth RMSE (lead=1)", ""]
        # header
        models = list(depth_profiles_lead1.keys())
        lines.append("| Depth (dbar) | " + " | ".join(models) + " |")
        lines.append("|---:|" + "|".join(["---:" for _ in models]) + "|")
        n_z = len(next(iter(depth_profiles_lead1.values())))
        for zi in range(n_z):
            d = depth_profiles_lead1[models[0]][zi]["depth_dbar"]
            vals = [f"{depth_profiles_lead1[m][zi]['rmse']:.3f}" for m in models]
            lines.append(f"| {d:.0f} | " + " | ".join(vals) + " |")
    md_path.write_text("\n".join(lines) + "\n", encoding="utf-8")

    plot_lead_skill(
        leads, rmse_series, "RMSE (µmol/kg)",
        f"Lead–RMSE · {region.get('id')} · {tag}", FIGS / f"lead_rmse_{tag}.png",
    )
    plot_lead_skill(
        leads, skill_series, "Skill vs persistence",
        f"Lead–Skill · {region.get('id')} · {tag}", FIGS / f"lead_skill_{tag}.png",
    )
    plot_lead_skill(
        leads, f1_series, "Hypoxia F1",
        f"Lead–Hypoxia F1 · {region.get('id')} · {tag}", FIGS / f"lead_hypoxia_f1_{tag}.png",
    )
    if 1 in st_maps:
        plot_spatial_rmse(
            st_maps[1], lat, lon,
